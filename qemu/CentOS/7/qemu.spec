...
sed -i 's@-liscsi@-liscsi -lgcrypt@g' $libiscsi_prefix/lib/pkgconfig/libiscsi.pc
export PKG_CONFIG_PATH=$libiscsi_prefix/lib/pkgconfig:%{_libdir}/pkgconfig
...
./build_configure.sh  \
  ...
  "%{optflags} -Wno-error=unused-result -Wno-error=unused-variable" \
%if 0%{have_fdt}
  enable \
%else
  disable \
